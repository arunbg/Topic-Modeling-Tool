...
public class CsvBuilder {
	...
	public void buildCsv1(String topicKeysFile,String outputCsv)
	{
		try
		{
			...
			{  
            	...
            	int tnum = Integer.parseInt(strArr[0])+1;
            	line = tnum + "," + strArr[2];            	
	...
	public void buildCsv3(String stateFile, int numDocsShown, String outputCsv)		//docs in topic
	{
		...
		for(int i=0;i<numTopics;i++){
			...
			for (int j=0;j<numDocsShown;j++){									//FIXME doc id and number are the same
				int k = idx[numDocs-j-1]+1;										//Descending
				
				int rank = j+1;
				line = (i+1)+","+rank+","+k+","+docNames.get(k-1)+"\n";
				out.write(line);
				//System.out.println(line);
			}
		}
	...
	public String dtLine2Csv(String line)
	{
		...
		{	int tn = Integer.parseInt(str[i])+1;
			csvLine = csvLine + "," + tn +","+String.format("%.3f",Float.parseFloat(str[i+1]));
	...
	public void buildCsv2(String docTopicsFile, String outputCsv)  //topics in doc
	{
		try
		{
            ...
            if(line!= null)
            {
            	FileWriter fwrite = new FileWriter(outputCsv);  
            	BufferedWriter out = new BufferedWriter(fwrite);
            	String header = "docId,filename,top topics, and contribution to doc ...";			//variable number of topics for each doc
/*            	for(int i=0;i<numTopics;i++){
            		header = header+",top-"+i;
            	}*/
            	out.write(header+"\n");
