		...
		try{
		    ...
		    while ((line = in.readLine()) != null)
	        {
				...
				for(int i =2;i<st.length-1;i=i+2){	
	        		...
	        		out.write(makeUrl("../Topics/Topic"+st[i]+".html",topics.get(Integer.parseInt(st[i])-1)));
	        		out.write(" ...</td></tr>");
	        	}
	...
	public void buildHtml3(File inputCsv, File outputDir)
	{
		...
		try{			
		    ...
		    while ((line = in.readLine()) != null)
	        {
				...
				out.write("<body><h4><u>TOPIC</u> : "+topics.get(Integer.parseInt(st[0])-1)+" ...</h4>");
				prevId = st[0];
	        	...
	        	out.write(String.format("<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td>%d.</td><td>%s</td><td>%s</td></tr>",Integer.parseInt(st[1])+1,"("+Ntd[Integer.parseInt(st[0])-1][Integer.parseInt(st[st.length-2])-1]+")",doc_name));
